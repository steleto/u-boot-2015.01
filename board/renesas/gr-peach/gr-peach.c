/*
 * Copyright (C) 2016 Atsushi Toyokura
...
int board_early_init_f(void)
{
	...
	pfc_set_gpio(6, 12, GPIO_OUT);
	pfc_set_gpio(6, 13, GPIO_OUT);
	pfc_set_gpio(6, 14, GPIO_OUT);
	pfc_set_gpio(6, 15, GPIO_OUT);
	gpio_set(6, 12, 1);
	gpio_set(6, 13, 1);
	gpio_set(6, 14, 1);
	gpio_set(6, 15, 1);
...
//	rtc_reset();	/* to start rtc */
	...
	/* =========== Pin Setup =========== */
	/* Specific for the RZ/H on the GR-PEACH board. Adjust for your board as needed. */

	/* Serial Console */
	pfc_set_pin_function(6, 2, ALT7, 0, 0); /* P6_2 = RxD2 */
	pfc_set_pin_function(6, 3, ALT7, 0, 0); /* P6_3 = TxD2 */
